...
- **Sparse-random-connectivity irregularity in spiking networks** — the spiking-network analogue of SCS-style chaos. [[brunel-1999-sparsely-connected-networks|Brunel (2000)]] shows that a sparsely connected inhibition-dominated E-I integrate-and-fire network occupies an [[Asynchronous Irregular State|asynchronous irregular (AI) state]] with CV ≈ 1, low cross-correlations, and Poisson-like spike statistics — even when the external input is a noiseless constant (σ_ext = 0). The irregularity is generated by the *quenched randomness of the sparse recurrent connectivity*: each neuron sees a different random subset of the population spike train, so each experiences a different realization of the recurrent fluctuations. The apparent noise in cortical spontaneous activity is, on this account, not inherited from channel stochasticity but produced by the graph structure of the network driving itself.
...
## Spikes are Events, not Aggregate Coding Variables

[[brette-2018-coding-metaphor|Brette (2018)]] sharpens what kinds of objects can have causal powers in the brain. Aggregate observables (firing rate averaged over a window, BOLD signal, trial-averaged spike counts) are not state variables of the brain's dynamical system — they cannot have causal effects, since causal effects must be attributable to events that *actually occur* on a given trial, not to averages over alternative trials. The Jennifer Aniston cell's "encoding" of the actress is a property of the trial-averaged spike rate; on any single trial the cell may not fire at all. *Percepts are experienced now, not on average*; trial-averaged firing rates therefore cannot underlie perception.

What survives as causally efficacious are *spike events* — discrete, timed actions in continuous time, each of which produces synaptic release, postsynaptic potentials, and downstream spike-time modulations. Brette's framing — "action potentials are *potentials that produce actions*, not hieroglyphs to be decoded" — is the cleanest statement of this position. For neural-noise theory, the implication is that noise should be characterized at the level of spike-time variability and ion-channel stochasticity, not at the level of trial-averaged rate fluctuations. Both deterministic-chaos sources (SCS, Brunel) and stochastic biophysical sources operate at the spike-event level; the apparent rate-level variability is downstream of these spike-time fluctuations. This is consistent with the wiki's existing framing but explicit only after Brette's argument.
...
- [[brunel-1999-sparsely-connected-networks|Brunel (2000)]] — spiking-network analogue: Poisson-like irregular firing in the AI state of sparsely connected E-I networks is generated by the random recurrent connectivity itself, not by external noise
- [[brette-2018-coding-metaphor|Brette (2018)]] — distinguishes spike events (causally efficacious, single-trial) from aggregate coding variables (rate averages, BOLD, population probabilities) which lack causal powers; "action potentials are potentials that produce actions, not hieroglyphs to be decoded."
