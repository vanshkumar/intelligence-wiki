...
**Limit cycles as control primitives.** Rotational dynamics is a cortical instance of a much older control-theoretic motif: [[Limit Cycle|limit-cycle attractors]] produce rhythmic output from steady input. [[brennan-2019-conserved-macroscopic-dynamics|Brennan & Proekt (2019)]]'s *C. elegans* manifold structure is another limit-cycle-like motif at the whole-nervous-system scale. The same dynamical object — a closed orbit in state space — shows up from worm locomotion to primate reach generation. A reasonable reading: **evolution re-uses limit cycles wherever rhythmic or time-structured motor output is required**, and genes constrain the conditions under which such cycles emerge ([[Degeneracy#Control-Theoretic Interpretation]]).

**Rate-level ancestor: Wilson-Cowan limit cycles.** The earliest explicit derivation of limit-cycle attractors in an E-I cortical population is [[wilson-cowan-1972-ei-populations|Wilson & Cowan (1972)]]. They showed that a two-variable mean-field E-I model exhibits a stable limit cycle whenever `c₁aₑ > c₄aᵢ + 18` (E-E coupling exceeds I-I coupling by a margin), with both frequency and amplitude of the oscillation monotonically increasing with input intensity. This is the canonical 2D mean-field ancestor of the higher-dimensional cortical rotations in motor cortex — the same dynamical object (a closed orbit encoding time-structured output), realized at different scales. Wilson-Cowan's Theorem 4 (limit-cycle-capable populations also exhibit [[Hysteresis|hysteresis]] under different bias) predicts that the same motor-cortex substrate that produces rotations during movement should also support persistent-activity-like states — consistent with the fixed-point preparatory states observed during the delay period ([[Computation Through Dynamics]]).
